...
public class RedissonProProvider extends CacheProvider {
    ...
    public void removeAll() {
        ...
        for (int i = 0; i<caches.length; i++) {
            groups.addAll(Arrays.asList(CacheLocator.getCache(caches[i].toString()).getGroups()));
        }
    ...
    public List<Map<String, Object>> getStats() {
        ...
        for (String group : groups) {
            RMap<String, Object> rmap = getCacheMap(group);
            ...
            stats.put("memory", (rmap instanceof RLocalCachedMap && rmap != null) ? rmap.size() : 0);
            stats.put("disk", (rmap != null) ? rmap.keySet().size() : 0);
            stats.put("configuredSize", new Integer(getConfigProperty(group,"cachesize","0")));
